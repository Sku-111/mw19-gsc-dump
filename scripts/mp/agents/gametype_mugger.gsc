...
{
    thread scripts\mp\bots\bots_gametype_mugger.gsc::bot_mugger_think();
}
...
agent_squadmember_mugger_think()
{
    ...
    if ( gettime() > self.next_time_check_tags )
    {
        ...
        if ( isdefined( var_1 ) )
        {
            var_2 = self.owner scripts\mp\bots\bots_gametype_mugger.gsc::bot_find_visible_tags_mugger( var_1, var_0 );
            self.tags_seen_by_owner = scripts\mp\bots\bots_gametype_conf.gsc::bot_combine_tag_seen_arrays( var_2, self.tags_seen_by_owner );
        }
    }

    self.tags_seen_by_owner = scripts\mp\bots\bots_gametype_conf.gsc::bot_remove_invalid_tags( self.tags_seen_by_owner );
    var_3 = scripts\mp\bots\bots_gametype_conf.gsc::bot_find_best_tag_from_array( self.tags_seen_by_owner, 0 );
